...
export function Storefront() {
  ...
  // 🔥 NEW: Add state for the search bar
  const [searchQuery, setSearchQuery] = useState("");

  // Dynamically generate categories from the products in the database
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      // Only return true if it matches both conditions
    ...
    <div className="flex-1 flex flex-col md:flex-row max-w-[1600px] mx-auto w-full">
      ...
      <aside className="hidden md:block w-64 lg:w-72 flex-shrink-0 border-r border-[#2A2A2A] p-6 h-[calc(100vh-73px)] sticky top-[73px] overflow-y-auto">
        ...
        <div className="relative mb-8">
          {/* 🔥 UPDATED: Connect input to searchQuery state */}
          <input 
            type="text" 
            value={searchQuery}
      ...
      <section className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto">
        ...
        <div className="md:hidden flex flex-col gap-4 mb-6">
          ...
          <div className="relative w-full">
            <input 
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search our cellar..."
              className="w-full bg-[#1A1A1A] border border-[#333] rounded-full text-sm text-[#ECECEC] px-10 py-3 outline-none focus:border-[#C27A2F] transition-colors"
            />
            <Search size={16} className="absolute left-4 top-3.5 text-[#666]" />
          </div>
        ...
        <h1 className="text-2xl md:text-3xl lg:text-4xl font-['Playfair_Display'] text-[#ECECEC] mb-6 md:mb-10 flex items-center justify-between">
          <span>Curated Excellence</span>
          {/* Display how many items matched the search */}
        ...
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-xl text-[#ECECEC] font-['Playfair_Display'] mb-2">Nothing found</h3>
            <p className="text-[#888]">We couldn't find any bottles matching "{searchQuery}"</p>
            <button 
              onClick={() => { setSearchQuery(""); setActiveCategory("All"); }}
              className="mt-6 text-[#C27A2F] hover:text-[#D4AF37] transition-colors text-sm"
            >
              Clear filters
            </button>
          </div>
              ...
              >
                ...
                <div className="p-5 flex flex-col flex-1 z-20 bg-gradient-to-t from-[#0D0D0D] to-transparent">
                  ...
                  <div className="flex flex-col">
                    ...
                    {/* Show stock remaining if it's low */}
                    {product.stock > 0 && product.stock <= 5 && (
                      <span className="text-xs text-red-400 mt-1">Only {product.stock} left!</span>
                    )}
                  </div>
                  ...
                  {product.stock > 0 ? (
                    ...
                    <div className="px-4 py-2 bg-red-500/10 border border-red-500/20 text-red-500 text-xs uppercase tracking-wider rounded-lg font-bold">
                      Sold Out
                    </div>
                  )}
                </div>
